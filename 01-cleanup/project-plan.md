...
| arc05-skipped-test-repair | Investigates and repairs inherited ignored tests, including refactoring or rewriting tested code when an ignored test exposes stale or defective behavior. | active; Slice05 open | arc04 closed locally at final cohesion baseline `7e2d8a7`. |
...
boundaries. Slice05 is open to route or replace the remaining inherited corpus
ignored tests through fast default corpus coverage while preserving full corpus
validation as an explicit policy command. CI policy remains reserved for the
following slice.
