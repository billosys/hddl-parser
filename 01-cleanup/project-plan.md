...
Opened Arc05 Slice06 GitHub Actions corpus policy. The slice is scoped to CI
workflow routing only: preserve the established Linux/macOS matrix and
checkout/toolchain choices, use locked Cargo commands, run fast/default corpus
coverage for branch pushes, run full corpus coverage for pull requests and
post-merge `main`/`master`, and explicitly disposition scheduled full-corpus
coverage.
...
Arc05 Slice05 locally closed corpus test routing. The remaining inherited IPC
and JSON corpus skips now run by default over the checked-in 43-case fast
selection, the full 900-case corpus command remains explicit and passed with
zero failures and zero JSON assertion failures, and GitHub Actions remained
unchanged. CDC verification is pending; the next planning boundary should be a
GitHub Actions corpus policy slice for branch-push, PR, scheduled, and
post-merge/main coverage.

### v2.29 - 2026-08-26

Opened Arc05 Slice05 for corpus test routing. The slice is scoped to removing
the remaining inherited Rust `#[ignore]` debt by routing IPC and JSON corpus
coverage through the verified fast corpus selection in default tests while
preserving full corpus validation as an explicit command. CI branch, PR,
scheduled, and post-merge policy remain a later slice.
