...
sed -n '1,620p' examples/corpus_measure.rs
...
rg -n "corpus_measure" Cargo.toml examples tests
...
- `examples/corpus_measure.rs` exists and compiles under locked all-target
  checks.
...
- `git diff -- .github/workflows Cargo.toml tests/integration_ipc.rs
...
|-----|-----------------|----------|
| M5-1 | reproduced | `examples/corpus_measure.rs` exists; `cargo check --locked --all-targets` passed; `rg -n "corpus_measure" Cargo.toml examples tests` finds only the example source comment; ignored-test scan still reports only the two corpus tests. |
...
| M5-7 | reproduced | Default locked all-target tests passed; the only remaining ignored tests are the two corpus tests; CI, Cargo manifest, and corpus integration test diffs are empty. |
...
The silent-drop diff is clean. The utility was added under `examples/`; it
records deterministic case IDs, per-phase timings, per-phase outcomes, exact
string equality, and structural JSON equality; it supports filter, limit, and
report path controls; and it writes ignored workbench evidence. The two corpus
