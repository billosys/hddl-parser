...
|----|-----------|--------|--------------|--------|--------|----------|-------|
| C8-1 | The slice diff is limited to Cargo reproducibility policy and generated lockfile state, on top of the verified Slice07 base. | `git diff --name-only e14078c..HEAD` unless Slice07 has merged upstream, then inspect changed files. | serious | RUST-006 | open | | Expected files are `Cargo.toml`, `.gitignore`, and `Cargo.lock` if tracked. |
...
| C8-5 | No runtime behavior, source cleanup, LSP, CLI, or public API change is mixed in. | `git diff --name-only e14078c..HEAD` unless Slice07 has merged upstream, then inspect non-Cargo files. | correctness | slice03-fix-map | open | | |
