...
|----|-----------|--------|--------------|--------|--------|----------|-------|
| C6-1 | The slice diff is limited to LSP error-boundary and metadata repair plus directly updated LSP tests. | `git diff --name-only main..HEAD` and inspect changed files. | serious | RUST-005/RUST-008 | open | | |
...
| C6-8 | RUST-004 lock-scope repair is not silently mixed into this slice unless explicitly promoted and recorded. | `git diff -- src/language_server/request_handler.rs` and inspect lock scope. | correctness | slice03-fix-map | open | | Prefer leaving RUST-004 to Slice07. |
