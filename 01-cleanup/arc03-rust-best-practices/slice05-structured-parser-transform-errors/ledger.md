...
|----|-----------|--------|--------------|--------|--------|----------|-------|
| C5-1 | The slice diff is limited to parser/transpiler/transform error handling and directly updated baselines, on top of the verified Slice04 CLI base. | `git diff --name-only f3a3f8d..HEAD` unless Slice04 has merged upstream, then inspect changed files. | serious | RUST-002/RUST-003 | open | | Slice04 includes operator-approved `.gitignore` workbench hygiene; do not count that as Slice05 scope. |
...
| C5-7 | The two Slice01 panic runtime probes no longer exit with panic code `101`. | Rerun the problem-as-domain and domain-only remove-equality runtime probes from the audit report. | serious | slice01-audit | open | | On the Slice04 base, the expected CLI outcome is ordinary non-zero `[Error]` handling rather than panic exit `101`. |
