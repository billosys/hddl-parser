...
| slice03-triage-and-fix-map | Convert the audit findings and baseline-test evidence into focused fix slices and upstream PR grouping. | Slice01 and Slice02. | CDC-verified. Created `fix-map.md`, opened Slice04-Slice08, and deferred RUST-007 to Arc04 public API cohesion work. |
| slice04-cli-error-exit-codes | Fix RUST-001 so recoverable `hddl_analyzer` CLI failures exit non-zero while success remains `0`. | Slice03. | CDC-verified. PR group 1: CLI error exits. Updates the CLI baselines in `tests/current_behavior.rs`; includes operator-approved `.gitignore` workbench hygiene. |
| slice05-structured-parser-transform-errors | Fix RUST-002 and RUST-003 by replacing recoverable parser/transpiler/transform panics with structured errors. | Slice04 preferred. | CDC-verified. PR group 2: structured parser and transform errors. Updates the panic-capture baselines in `tests/current_behavior.rs`; former panic probes now exit through Slice04's ordinary `[Error]` path. |
| slice06-lsp-error-boundaries-and-metadata | Fix RUST-005 and RUST-008 by making ordinary LSP request/runtime failures non-panicking and using package metadata for initialize version. | Slice03; may run after Slice05 for review order. | CDC-verified. PR group 3a: LSP error boundaries and metadata. Updates and extends `tests/lsp_current_behavior.rs`; RUST-004 remains for Slice07. |
| slice07-lsp-diagnostic-lock-scope | Fix RUST-004 by dropping the diagnostic document-map read guard before awaited work. | Slice06 preferred if shared LSP helpers emerge. | CDC-verified with C7-5 deferred. PR group 3b: LSP diagnostic lock scope. Source proof confirms the guard is dropped before awaited work; deterministic runtime contention coverage has a re-entry condition. |
| slice08-cargo-reproducibility-policy | Fix RUST-006 by replacing wildcard dependency requirements and settling `Cargo.lock` tracking policy. | Behavior repair slices preferred first. | CDC-verified. PR group 4: Cargo reproducibility policy. Tracks Cargo's generated `Cargo.lock`, keeps the broad `*.lock` ignore rule with a `!Cargo.lock` exception, and replaces wildcard dependency requirements. |
...
Slice08 CDC verification landed. RUST-006 is fixed by replacing wildcard
dependency requirements and tracking Cargo's generated lockfile for the
binary-bearing package. All production repair slices opened by Slice03 are now
CDC-verified; Arc03 is ready for arc-level closure.

### v1.9 - 2026-08-26

Slice07 CDC verification landed. RUST-004 is fixed by cloning the diagnostic
document before awaited work; deterministic runtime contention coverage is
deferred with a concrete re-entry condition. Slice08 Cargo reproducibility
policy is next.

### v1.8 - 2026-08-26

Slice06 CDC verification landed. RUST-005 and RUST-008 are fixed for the
scoped LSP request-boundary and initialize-metadata failures; RUST-004 remains
intentionally isolated for Slice07.

### v1.7 - 2026-08-26

Slice05 CDC verification landed. RUST-002 and RUST-003 are fixed for the
mapped parser/transpiler/transform panic paths, and the former panic probes now
exit through Slice04's ordinary CLI error path. Slice06 LSP error boundaries
and metadata is next.

### v1.6 - 2026-08-25

Slice04 CDC verification landed. RUST-001 is fixed: recoverable
`hddl_analyzer` CLI failures exit non-zero, successful verification remains
`0`, and RUST-002/RUST-003 panic baselines remain intentionally unchanged for
Slice05. The feature diff includes operator-approved `.gitignore` workbench
hygiene.

### v1.5 - 2026-08-25

Slice03 CDC verification landed. Arc03 now has a verified fix map, five
focused repair slices opened, and RUST-007 routed to Arc04 public API cohesion
with explicit re-entry conditions.
